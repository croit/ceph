...
RGWCoroutine::~RGWCoroutine() {
  ldout(cct, 20) << "op dtor " << " this: " << (void *)this
...
{
  ldout(cct, 20) << "dtor " << " this: " << (void *)this
                           << " spawned:" << spawned.entries.size()
                           << dendl;
  for (auto op : ops) {
    op->put();
...
int RGWCoroutinesStack::operate(const DoutPrefixProvider *dpp, RGWCoroutinesEnv *_env)
{
  ...
  if (r < 0) {
    ldpp_dout(dpp, 20) << *op << ": operate() returned r=" << r
...
RGWCoroutinesStack *RGWCoroutinesStack::spawn(RGWCoroutine *source_op, RGWCoroutine *op, bool wait)
{
  ...
  ldout(cct, 20) << __func__ << " parent: " << (void *)this
                            << " child: " << (void *)stack
                            << " op: " << (void *)op
...
int RGWCoroutinesManager::run(const DoutPrefixProvider *dpp, list<RGWCoroutinesStack *>& stacks)
{
  ...
  for (list<RGWCoroutinesStack *>::iterator iter = scheduled_stacks.begin(); iter != scheduled_stacks.end() && !going_down;) {
    ...
    if (stack->is_io_blocked()) {
      ...
      ldout(cct, 20) << __func__ << ":" << " stack=" << (void *)stack << " is done, "
	...
	if (!s->is_blocked_by_stack() && !s->is_done()) {
	  if (s->is_io_blocked()) {
	    ...
	    blocked_count++;
            ...
            ldout(cct, 20) << __func__ << ":" << " stack=" << (void *)stack
                                              << " unblocked s=" << (void *)s
                           << dendl;
	    s->_schedule();
	  }
      ...
      op_not_blocked = true;
      stack->run_count++;
      ldout(cct, 20) << __func__ << ": scheduling " << " stack=" << (void *)stack
                     << dendl;
      stack->_schedule();
    }
                                  ...
                                  std::optional<std::function<int(uint64_t stack_id, int ret)> > cb)
{
  ...
  ldout(cct, 20) << __func__ << " op:" << (void*)this << " spawned:" << spawned.entries.size()
                << " num_cr:" << num_cr_left
                << " done:" << done << dendl;
