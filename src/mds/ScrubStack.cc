                                        ...
                                        ScrubHeaderRef& header)
{
  ...
  add_remote_link_damage(path, head_path, ino, header);
}

void ScrubStack::add_remote_link_damage(const std::string &path, const std::string& head_path,
...
void ScrubStack::remote_link_checkup(CDentry *dn, ScrubHeaderRef &header, bool *added)
{
  ...
  if (!remote_inode) {
    if (mdcache->mds->damage_table.is_remote_damaged(remote_ino)) {
      ...
      add_remote_link_damage(get_dn_path(dn), "", remote_ino, header);
      return ;
    }
    ...
    _enqueue(remote_inode, header, true, added,
      { std::make_pair(std::move(get_dn_path(dn)), remote_ino) });
  }
}
                                               ...
                                               CDentry* dn)
{
  ...
  if (r >= 0) {
    ...
    CInode* remote_inode = mdcache->get_inode(remote_ino);
    if (!remote_inode) {
      add_remote_link_damage(path, "", remote_ino, header);
    } else {
      _enqueue(remote_inode, header, true, nullptr,
	{ std::make_pair(std::move(path), remote_ino) });
    }
  } else {
